...
| Image | `IconImage` exposes source, asynchronous, status, mipmap, backer and exact sizing at `widgets/IconImage.qml:26-66`. | Synchronous size-keyed decode, stretch/fit/crop, and cached binary alpha-mask distance fields at `crates/mold-image/src/image_cache.rs`, `distance_field.rs`, and `crates/mold-scene/src/schema.rs`. | **partial.** Observable ready/error state, reload/invalidation, mirror/auto-transform/source crop and animated images are absent. Distance-field masks are an additional native rendering mode, not lifecycle parity. |
...
| Shapes | QtQuick Shapes is available to configurations. | SVG path strings plus Polymorpher-backed topology-matched rounded-polygon morphs are native at `crates/mold-scene/src/schema.rs` and `crates/mold-render/src/path.rs`. | **partial.** Morphing covers named rounded polygons, not arbitrary path topology. No reactive ShapePath element tree, arcs/segments, caps/joins/miter/dash, gradient fills/strokes, renderer choice, or status. |
...
| Animation | QtQuick supplies animation objects, groups, pause/path/spring/smoothed lifecycle. | Animato-backed Rust tween/spring state, Mold smoothed motion, retargeting, and rotation direction exist in `crates/mold-scene/src/animation.rs`, `motion.rs`, and `scene.rs`. | **partial.** Mold retains the Rust compositor clock. Animato timeline facilities are not exposed: running/paused/loops, groups, pause/script/path, completion/cancellation, and per-type lifecycle remain absent. |
