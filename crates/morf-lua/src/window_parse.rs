...
pub(crate) fn window_root<'gc>(
    ...
    let LuaValue::UserData(root) = options.get_value(ctx, "root") else {
        return Err("window root must be a morf node".into());
    };
    root.downcast_static::<NodeToken>()
        .map(|root| root.handle)
        .map_err(|_| "window root must be a morf node".into())
}
...
pub(crate) fn window_parent(value: LuaValue<'_>, field: &str) -> Result<Option<u64>, String> {
    match value {
        ...
        LuaValue::UserData(parent) => parent
            ...
            .map_err(|_| format!("{field} must be a morf window surface")),
        _ => Err(format!("{field} must be a morf window surface")),
    }
}
        ...
        match anchor.get_value(ctx, "node") {
            ...
            LuaValue::UserData(node) => {
                let node = node
                    ...
                    .map_err(|_| "popup anchor node must be a morf node".to_owned())?
                    .handle;
            ...
            _ => return Err("popup anchor node must be a morf node".into()),
        }
    } else {
        None
    };
...
pub(crate) fn parse_floating_surface<'gc>(
    ...
    let title = table_string(ctx, options, "title", "morf")?;
    let app_id = table_string(ctx, options, "app_id", "morf")?;
