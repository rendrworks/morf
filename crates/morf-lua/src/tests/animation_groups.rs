...
fn lua_plays_a_sequence_of_property_steps() {
    ...
    assert_eq!(frame.groups[0].end, morf_scene::AnimationEnd::Completed);
    ...
    runtime
        .execute(
            ...
            br#"
                local morf = require("morf")
                local ui = require("morf.ui")
                local handle = morf.animation.play {
                    loops = "forever",
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                morf.ipc["throw"] = function(velocity)
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local ui = require("morf.ui")
                local panel = ui.Rect { width = 100, height = 100, x = 0 }
                morf.ipc["throw"] = function()
                  morf.animation.fling {
                    node = panel, property = "x", velocity = 300,
                    friction = 0, min_velocity = 0,
                  }
                end
                morf.ipc["push"] = function(delta)
                  return morf.animation.impulse(panel, "x", delta)
                end
            "#,
        )
        .unwrap();
