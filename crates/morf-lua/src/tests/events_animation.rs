    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local ui = require("morf.ui")
                local status = morf.signal("touch.status", "idle")
                ui.MouseArea {
                  width = 100,
                  height = 100,
    ...
    runtime
        .execute(
            ...
            br#"
                local morf = require("morf")
                local ui = require("morf.ui")
                local status = morf.signal("wheel.status", "idle")
                ui.MouseArea {
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local model = morf.list_model(items)
                local view = morf.virtual_list(model, 40, 400, 1)
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local view = morf.virtual_list(model, 20, 100, 0)
                view:sync()
                model:replace({
                  { id = "b", label = "changed" },
                  { id = "d", label = "new" },
