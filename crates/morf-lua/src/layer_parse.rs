...
///
/// `morf.surface.margin_left = x` is exactly the assignment a slide animation
...
///
/// `morf.surface.<key> = value` and `window.layer { <key> = value }` are the
...
pub(crate) fn apply_layer_setting<'gc>(
    ...
    match key {
        ...
        "backdrop" => {
            ...
            Ok(assign_layer_setting(&mut config.backdrop, Some(value)))
        }
        "session_lock" => {
            let LuaValue::Boolean(value) = value else {
...
pub(crate) fn parse_layer_surface<'gc>(
    ...
    for (key, value) in options.iter(ctx) {
        ...
        if key == "reserve" {
            return Err("reserve is only available on morf.surface".into());
        }
