...
pub(crate) fn install_animation_api<'gc>(
    ...
    morf.set_field(ctx, "animation", animation);
}

/// Installs `morf.easing`, direct evaluation of a timing curve.
///
...
pub(crate) fn install_easing_api<'gc>(ctx: Context<'gc>, morf: Table<'gc>) {
    ...
    morf.set_field(ctx, "easing", easing);
}
