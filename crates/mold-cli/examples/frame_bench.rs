...
//!
//! ```sh
//! cargo run --release -p mold-cli --example frame_bench -- examples/quickshell/init.lua
//! ```
//!
...
use mold_layout::{Layout, Size, TextMeasurer, TextOptions};
use mold_lua::{Limits, Runtime, Screen};
use mold_render::DrawList;
...
fn main() {
    ...
    // write a configuration is `mold.variants(mold.screens, …)`, which builds
    ...
    println!(
        ...
        std::mem::size_of::<mold_render::DrawCommand>(),
        std::mem::size_of::<mold_render::DrawCommand>() * nodes / 1024
    );
