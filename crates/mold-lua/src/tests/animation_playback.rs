    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local target = mold.signal("target", 1)
                ui.Item {
                    behavior = {
                        opacity = {
                            duration = 100,
...
fn lua_receives_the_behavior_finished_handler_with_a_reason() {
    ...
    assert_eq!(frame.events[0].end, mold_scene::AnimationEnd::Completed);
...
fn lua_controls_playback_of_a_running_animation() {
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                local ok, err = target:set(100)
                assert(ok, err)
                assert(mold.animation.active(node, "x"))
                assert(not mold.animation.paused(node, "x"))
            "#,
        )
        .unwrap();
    let node = runtime.scene().roots()[0];
    ...
    runtime
        .execute(
            ...
            br#"
                local mold = require("mold")
                assert(mold.animation.pause(_G.node, "x"))
                assert(mold.animation.paused(_G.node, "x"))
            "#,
        )
        .unwrap();
    runtime.tick_animations(Duration::from_millis(100)).unwrap();
    ...
    runtime
        .execute(
            ...
            br#"
                local mold = require("mold")
                assert(mold.animation.resume(_G.node, "x"))
                assert(mold.animation.finish(_G.node, "x"))
                assert(not mold.animation.active(_G.node, "x"))
                -- Nothing is running any more, so the controls report so
                -- instead of raising.
                assert(not mold.animation.stop(_G.node, "x"))
                assert(mold.animation.progress(_G.node, "x") == nil)
            "#,
        )
        .unwrap();
    assert_eq!(runtime.scene().number(node, "x").unwrap(), 100.0);
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                assert(mold.animation.set_enabled(node, "x", false))
                local ok, err = target:set(100)
                assert(ok, err)
                assert(not mold.animation.active(node, "x"))
                assert(mold.animation.set_enabled(node, "x", true))
                ok, err = target:set(0)
                assert(ok, err)
                assert(mold.animation.active(node, "x"))
            "#,
    ...
    runtime
        .execute(
            ...
            br##"
                ...
                local eased = mold.easing.value("in_out_cubic", 0.3)
                local between = mold.easing.number("in_out_cubic", 0.3, 0, 100)
                ...
                local point = mold.easing.point("out_quad", 0.4, { x = 0, y = 0 }, { x = 200, y = 100 })
                assert(math.abs(point.x / 2 - point.y) < 1e-9, "point axes drifted apart")

                local rect = mold.easing.rect(
                    "linear", 0.5,
                ...
                local bezier = mold.easing.value({ x1 = 0.4, y1 = 0, x2 = 0.2, y2 = 1 }, 0.5)
                assert(bezier > 0 and bezier < 1, "bezier out of range")

                local color = mold.easing.color("linear", 0.5, "#000000", "#ffffff")
                assert(math.abs(color.r - 0.5) < 1e-6, "colour did not interpolate")
                assert(color.a == 1, "colour lost its alpha")

                local ok = pcall(mold.easing.value, "not_a_curve", 0.5)
