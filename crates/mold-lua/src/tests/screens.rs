...
fn a_runtime_with_no_compositor_keeps_the_screens_it_was_built_with() {
    ...
    runtime
        .execute(
            "no-outputs.lua",
            br#"
                assert(#mold.screens == 1)
                assert(mold.screens[1].name == "eDP-1")
            "#,
        )
        .unwrap();
}
    ...
    runtime
        .execute(
            ...
            br#"
                ...
                assert(#mold.screens == 2)
                assert(mold.screens[1].width == 1920)
                assert(mold.screens[2].width == 2560)
            "#,
        )
        .unwrap();
}
