...
local lanes = {
  {
    ...
    driven(0, { duration = 900, easing = "in_out_cubic" }, theme.accent),
  },
  {
    "Spring",
    "a force; overshoots, then settles",
    driven(0, { kind = "spring", mass = 1, damping = 12, stiffness = 140, epsilon = 0.05 }, theme.accent),
  },
  {
    "Smoothed",
    "constant speed, however far it is",
    driven(0, { kind = "smoothed", velocity = 620 }, theme.accent),
  },
...
for index, lane in ipairs(lanes) do
  ...
  rows[#rows + 1] = ui.Rect {
    x = 20, y = top - 12, width = W - 40, height = ROW_H + 14, radius = 14, color = theme.panel,
  }
  rows[#rows + 1] = ui.Text {
    x = 44, y = top + 6, width = 190, text = lane[1], font_size = 16, color = theme.accent,
  }
  rows[#rows + 1] = ui.Text {
    x = 44, y = top + 30, width = 200, text = lane[2], font_size = 11, wrap = true, line_height = 1.4, color = theme.muted,
  }
  rows[#rows + 1] = ui.Rect {
